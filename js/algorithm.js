	...
	run : function(pfu){
		...
		function goStream(stream){
			...
			javascriptNode.onaudioprocess = function () {
				...
				/*DELETE - THIS IS JUST FOR MONITORING*/
				//Call graphy into action
				graph.drawFrequencySum();
				displayData.startTextb();
				displayValuse();
			}
			...
			//when we get max and min variances we check if next 4(onFastNumOfVar) variances are bigger than b.minVariance.
				...
				if(b.vcd == b.onFastNumOfVar){
					if (b.windowArray.every(CheckOn)){
						b.functionRunning = true;
						breathingEvents.whichEvent = 'FAST;'	//DELETE for algorithm
						doEvent.start();
					}
					else{
		...
		//SLOW
			...
			if(b.pcd < 35){
				...
				b.functionRunning = true;
				breathingEvents.whichEvent = 'SLOW;'	//DELETE for algorithm
				doEvent.start();
				b.pcd = 0;
			}
		}
